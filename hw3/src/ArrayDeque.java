...
public class ArrayDeque<T> {
    ...
    public void addFirst(T data) {
        if (data == null) {
            ...
            for (int i = 0; i < size; i++) {
                newBackingArray[i + 1] = backingArray[front];
                front++;
    ...
    public void addLast(T data) {
        if (data == null) {
            ...
            for (int i = 0; i < backingArray.length; i++) {
                int x = (front++) % backingArray.length;
                newBackingArray[i] = backingArray[x];
            }
            front = 0;
            backingArray = newBackingArray;
        }
    ...
    public T removeFirst() {
        if (size == 0) {
            throw new NoSuchElementException("There is no data in the deque.");
        }
    ...
    public T removeLast() {
        ...
        int indexRemoved = mod(front + size - 1, backingArray.length);
        T removed = backingArray[indexRemoved];
        backingArray[indexRemoved] = null;
        if (size == 1) {
            front = 0;
            size = 0;
        } else {
